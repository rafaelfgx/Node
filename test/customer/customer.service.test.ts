...
describe("CustomerService", () => {
    ...
    describe("List", () => {
        ...
        it("should return list with actual repository base", async () => {
            const actualRepository = new CustomerRepository();
            vi.spyOn(actualRepository, "list").mockResolvedValue([Data.customer]);
            const result = await new CustomerService(actualRepository).list();
            expect(actualRepository.list).toHaveBeenCalledTimes(1);
            expect(result).toEqual([Data.customer]);
        });
